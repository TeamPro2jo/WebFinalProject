...
CREATE SEQUENCE chattroom_seq NOCACHE;

CREATE TABLE chattroom(
	roomid NUMBER,
	creator NBARCHAR2(64),
	participant NVARCHAR2(64)
);

ALTER TABLE chattroom ADD CONSTRAINT chattroom_roomid_PK PRIMARY KEY(roomid);
ALTER TABLE chattroom ADD CONSTRAINT chattroom_creator_FK FOREIGN KEY(creator) REFERENCES account(id);
ALTER TABLE chattroom ADD CONSTRAINT chattroom_participant_FK FOREIGN KEY(participant) REFERENCES account(id);
...
CREATE TABLE message(
	...
	sendtime DATETIME DEFAULT SYSDATE,
	roomid NUMBER
);
...
ALTER TABLE message ADD CONSTRAINT message_roomid_FK FOREIGN KEY(roomid) REFERENCES chattroom(roomid);
