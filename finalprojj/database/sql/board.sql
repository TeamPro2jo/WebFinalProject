...
INSERT INTO board_type VALUES(1, '1 게시판');
INSERT INTO board_type VALUES(2, '2 게시판');
INSERT INTO board_type VALUES(3, '3 게시판');
...
COMMENT ON COLUMN board.cdate IS '게시글 생성일';
COMMENT ON COLUMN board.udate IS '게시글 수정일';
...
ALTER TABLE attach_files ADD CONSTRAINT attach_files_fid_PK PRIMARY KEY(fid);
ALTER TABLE attach_files ADD CONSTRAINT attach_files_bid_FK FOREIGN KEY(bid) REFERENCES board(fid);
ALTER TABLE attach_files MODIFY fname CONSTRAINT attach_files_name_NN NOT NULL;
...
COMMENT ON COLUMN attach_files.fid IS '첨부파일 아이디(식별번호)';
COMMENT ON COLUMN attach_files.bid IS '게시글 아이디';
COMMENT ON COLUMN attach_files.fname IS '첨부파일의 파일명';
